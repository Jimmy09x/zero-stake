...
{
    return GetBoolArg("-testnet", false) ? 2929 : 2929;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop ZEROSTAKE server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "ZEROSTAKE server stopping";
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: zerostake-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("zerostake-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use zerostaked";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("zerostake-rpchand");
